...
class List extends Component {
  ...
  editProductModal = (id) => {
    ...
    const selectedProduct = this.props.products.filter(product => product.id === id)[0];
  ...
  render() {
    const { products, modalOpen, selectedProduct, selectedProductFormData } = this.props;
    return (
            ...
            { products.map((product, index) => {
              return (
                <Col md={4} lg={4} key={index} className="mt-4">
                  ...
                  deleteClicked={() => this.delete(product.id)}></Product>
                </Col>
              );
            })}
          </Row>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    selectedProduct: (payload) => dispatch(productAction.selectedProduct(payload)),
    editProduct: (payload, docId) => dispatch(productAction.editProductRequest(payload, docId)),
